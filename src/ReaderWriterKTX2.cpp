...
static osg::ref_ptr<osg::Image> buildFaceImage(
    ktxTexture2* ktx, uint32_t face, const GLFormat& fmt)
{
    ...
    img->setImage(w, h, 1,
        static_cast<GLint>(fmt.internal), fmt.pixel, fmt.type,
        buf, osg::Image::USE_NEW_DELETE);
    if (!mipmapOffsets.empty())
        img->setMipmapLevels(mipmapOffsets);
    return img;
}
...
class ReaderWriterKTX2 : public osgDB::ReaderWriter
{
    static ReadResult loadKTX2(const std::string& file)
    {
        ...
        if (rc != KTX_SUCCESS) {
            OSG_WARN << "ReaderWriterKTX2: failed to open " << file
                     << ": " << ktxErrorString(rc) << "\n";
        ...
        if (!vkFormatToGL(ktx->vkFormat, fmt)) {
            OSG_WARN << "ReaderWriterKTX2: unsupported VkFormat " << ktx->vkFormat
                     << " in " << file << "\n";
            return ReadResult::FILE_NOT_HANDLED;
        }

        if (fmt.compressed) {
            OSG_WARN << "ReaderWriterKTX2: BC-compressed KTX2 not yet supported: " << file << "\n";
            return ReadResult::FILE_NOT_HANDLED;
        }
    ...
    static WriteResult saveCubeMap(const osg::TextureCubeMap* texcm, const std::string& file)
    {
        ...
        if (!face0 || !face0->data()) {
            OSG_WARN << "ReaderWriterKTX2: write: face 0 has no CPU image data\n";
            return WriteResult::ERROR_IN_WRITING_FILE;
        }

        ktx_uint32_t vkFmt;
        if (!glToVkFormat(face0->getInternalTextureFormat(), vkFmt)) {
            OSG_WARN << "ReaderWriterKTX2: write: unsupported GL internal format "
                     << face0->getInternalTextureFormat() << "\n";
            return WriteResult::ERROR_IN_WRITING_FILE;
        }
        ...
        if (rc != KTX_SUCCESS) {
            OSG_WARN << "ReaderWriterKTX2: ktxTexture2_Create: " << ktxErrorString(rc) << "\n";
            return WriteResult::ERROR_IN_WRITING_FILE;
        }
        struct Guard { ktxTexture2* p; ~Guard() { ktxTexture_Destroy((ktxTexture*)p); } } g{ktx};
        ...
        for (uint32_t face = 0; face < 6; ++face) {
            ...
            if (!img || !img->data()) {
                OSG_WARN << "ReaderWriterKTX2: write: face " << face << " has no CPU data\n";
                return WriteResult::ERROR_IN_WRITING_FILE;
            }
            for (uint32_t mip = 0; mip < numMips; ++mip) {
                ktx_size_t mipBytes = ktxTexture_GetImageSize((ktxTexture*)ktx, mip);
                rc = ktxTexture_SetImageFromMemory(
                    (ktxTexture*)ktx, mip, 0, face, getMipPtr(img, mip), mipBytes);
                if (rc != KTX_SUCCESS) {
                    OSG_WARN << "ReaderWriterKTX2: SetImageFromMemory mip=" << mip
                             << " face=" << face << ": " << ktxErrorString(rc) << "\n";
                    return WriteResult::ERROR_IN_WRITING_FILE;
                }
            }
        }
        ...
        if (rc != KTX_SUCCESS) {
            OSG_WARN << "ReaderWriterKTX2: WriteToNamedFile: " << ktxErrorString(rc) << "\n";
            return WriteResult::ERROR_IN_WRITING_FILE;
        }
        OSG_NOTICE << "ReaderWriterKTX2: wrote cubemap KTX2 " << file << "\n";
    ...
    {
        if (!img->data()) {
            OSG_WARN << "ReaderWriterKTX2: write: image has no CPU data\n";
            return WriteResult::ERROR_IN_WRITING_FILE;
        }
        ...
        for (uint32_t mip = 0; mip < numMips; ++mip) {
            ktx_size_t mipBytes = ktxTexture_GetImageSize((ktxTexture*)ktx, mip);
            rc = ktxTexture_SetImageFromMemory(
                (ktxTexture*)ktx, mip, 0, 0, getMipPtr(img, mip), mipBytes);
            if (rc != KTX_SUCCESS) {
                OSG_WARN << "ReaderWriterKTX2: SetImageFromMemory mip=" << mip
                         << ": " << ktxErrorString(rc) << "\n";
                return WriteResult::ERROR_IN_WRITING_FILE;
        ...
        OSG_NOTICE << "ReaderWriterKTX2: wrote 2D KTX2 " << file << "\n";
        return WriteResult::FILE_SAVED;
    }
...
public:
    ...
    WriteResult writeObject(const osg::Object& obj, const std::string& file,
    ...
    WriteResult writeImage(const osg::Image& img, const std::string& file,
                           const osgDB::Options*) const override
    {
        if (!acceptsExtension(osgDB::getLowerCaseFileExtension(file)))
            return WriteResult::FILE_NOT_HANDLED;
