...
// Everything returned by createPBRIBLScene(). lutCamera must be added to a rendered scene graph so
// its PRE_RENDER pass can populate the BRDF LUT. The textures remain available for reuse by other
// renderer-owned state, such as a skybox.
struct PBRIBLScene {
...
// the node and must add the returned lutCamera to a rendered scene graph.
