// osggltf-iblbake-gpu -- bakes prepared IBL resources to KTX2.
//
// Usage:
//   osggltf-iblbake-gpu <input.hdr> <output.ktx2>
...
//
//   osggltf-iblbake-gpu --brdf-lut-only <output.ktx2> [--lut-size N]
...
int runBRDFLUTOnly(const std::string& outputPath, int lutSize) {
	...
	if(!lut.camera) {
		...
		OSG_WARN << "osggltf-iblbake-gpu: BRDF LUT unexpectedly already baked in this process" << std::endl;
	...
	if(!readback->isDone()) {
		OSG_WARN << "osggltf-iblbake-gpu: BRDF LUT bake did not complete" << std::endl;
...
int main(int argc, char* argv[]) {
	if(argc > 1 && std::string(argv[1]) == "--brdf-lut-only") {
		if(argc < 3) {
			OSG_WARN << "Usage: osggltf-iblbake-gpu --brdf-lut-only <output.ktx2> [--lut-size N]" << std::endl;
	...
	if(!specularScene.root) {
		OSG_WARN << "osggltf-iblbake-gpu: failed to build specular bake scene" << std::endl;
	...
	if(!diffuseScene.root) {
		OSG_WARN << "osggltf-iblbake-gpu: failed to build diffuse bake scene" << std::endl;
	...
	if(!specularScene.readback->isDone()) {
		OSG_WARN << "osggltf-iblbake-gpu: specular bake readback did not complete" << std::endl;

		return 1;
	}

	if(!diffuseReadback->isDone()) {
		OSG_WARN << "osggltf-iblbake-gpu: diffuse bake readback did not complete" << std::endl;
	...
	if(!specularResult || !osgDB::writeObjectFile(*specularResult, specularPath)) {
		OSG_WARN << "osggltf-iblbake-gpu: failed to write " << specularPath << std::endl;

		return 1;
	}

	OSG_NOTICE << "osggltf-iblbake-gpu: wrote " << specularPath << std::endl;
	...
	if(!diffuseResult || !osgDB::writeObjectFile(*diffuseResult, diffusePath)) {
		OSG_WARN << "osggltf-iblbake-gpu: failed to write " << diffusePath << std::endl;

		return 1;
	}

	OSG_NOTICE << "osggltf-iblbake-gpu: wrote " << diffusePath << std::endl;
